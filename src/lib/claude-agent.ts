...
async function createApplication(job: JobListing): Promise<Application> {
  await log('application_started', `Starting application for ${job.title} at ${job.company}`)
  
  await log('cover_letter_generated', 'Generating personalized cover letter...')
  ...
  await log('application_submitted', `Application submitted for ${job.title} at ${job.company}`, {
...
export async function runJobHuntingCycle(): Promise<{
  ...
  await setAgentState({ status: 'searching', currentTask: 'Searching for new job listings...' })
  cycleLogs.push(await log('search_started', 'Starting new job search cycle...'))
  ...
  try {
    ...
    if (jobs.length === 0) {
      ...
      cycleLogs.push(await log('thinking', 'No live jobs found, using demo data for testing'))
    }
  } catch (error) {
    jobs = generateMockJobs(5)
    cycleLogs.push(await log('error', 'Error fetching jobs, using demo data'))
  }
  
  cycleLogs.push(await log('job_found', `Found ${jobs.length} potential job listings`))
  ...
  if (newJobs.length === 0) {
    cycleLogs.push(await log('thinking', 'No new jobs to apply to - all listings already processed'))
    await setAgentState({ status: 'idle', currentTask: 'Waiting for new listings...' })
    return { jobsFound: jobs.length, applicationsSubmitted: 0, logs: cycleLogs }
  }
  
  await setAgentState({ status: 'applying', currentTask: 'Analyzing job listings...' })
  
  let applicationsSubmitted = 0
  
  // Process each new job
  for (const job of newJobs.slice(0, 3)) { // Limit to 3 applications per cycle
    await saveDiscoveredJob(job)
    
    cycleLogs.push(await log('job_analyzed', `Analyzing: ${job.title} at ${job.company}`))
    
    const analysis = await analyzeJobFit(job)
    
    if (analysis.shouldApply) {
      cycleLogs.push(await log('thinking', `Good fit (score: ${analysis.score}/100) - preparing application`))
      
      try {
        await createApplication(job)
        applicationsSubmitted++
        cycleLogs.push(await log('application_submitted', `✓ Applied to ${job.title} at ${job.company}`))
      } catch (error) {
        cycleLogs.push(await log('error', `Failed to submit application: ${error}`))
      }
    } else {
      cycleLogs.push(await log('thinking', `Skipping (score: ${analysis.score}/100) - ${analysis.reasons[0]}`))
    }
  ...
  await setAgentState({ status: 'waiting', currentTask: 'Waiting for responses...' })
  cycleLogs.push(await log('status_update', `Cycle complete: ${applicationsSubmitted} applications submitted`))
